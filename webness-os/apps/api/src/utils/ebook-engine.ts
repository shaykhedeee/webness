...
2. What are the common gaps, deficiencies, or major reader pain points that are usually left unaddressed? (e.g. what are readers complaining about in standard Kindle book reviews for this niche?)
...
Format as a structured Markdown dossier.`;
  ...
  for (let i = 0; i < outline.chapters.length; i++) {
    ...
    // Build deep narrative continuity context from ALL previously drafted chapters
    let narrativeContext = "";
    if (i > 0) {
      const allPreviousSummaries = draftedChapters.map(c => 
        `Chapter ${c.number} (${c.title}): ${c.keyTakeaways.join(", ")}`
      ).join("\n");
      
      const prevCh = draftedChapters[i - 1];
      
      // Extract a larger snippet of the end of the previous chapter (last ~800 words)
      const prevWords = prevCh.content.split(/\s+/);
      const endingSnippet = prevWords.slice(-800).join(" ");
      
      narrativeContext = `PREVIOUSLY ESTABLISHED CONTEXT (ALL CHAPTERS SO FAR):
${allPreviousSummaries}

IMMEDIATE PREVIOUS CHAPTER (Chapter ${prevCh.number}): "${prevCh.title}"
...
... ${endingSnippet}`;
    ...
    try {
      const drafted = await draftSingleChapter(ch, outline, config, callers, researchDossier, narrativeContext, onProgress);
...
Write the complete chapter now:`;
  ...
  // --- RECURSIVE CRITIQUE & REVISION ---
  if (onProgress) {
    onProgress("drafting", `Critiquing Draft of Chapter ${chapterOutline.number} (Recursive Pass)…`, 0); // use 0 or undefined to not jump progress
  }
  
  const critiquePrompt = `You are a strict editorial director. You must critique this draft for Chapter ${chapterOutline.number}.
Does the draft sound too AI-generated? Does it repeat concepts? Does it lack the necessary tone (${config.tone})?
Provide 3-4 bullet points of strict, actionable feedback to improve it.
DRAFT CONTENT:
${content}`;
  
  let critique = "";
  try {
    critique = await callers.flash(critiquePrompt);
  } catch {
    critique = "";
  }

  if (critique && onProgress) {
    onProgress("drafting", `Revising Chapter ${chapterOutline.number} based on Critique…`, 0);
    const revisePrompt = `You are a bestselling author. Revise this chapter based on the following critique.
...
Write the final, revised chapter in Markdown. Keep all required formatting. Do not include chapter prefixes.`;
    try {
       content = await callers.pro(revisePrompt, "gemini-2.5-pro");
    } catch {
